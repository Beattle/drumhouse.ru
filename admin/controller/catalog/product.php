	...
	private $error = array(); 
  	...
  	private function getList() {
		...
		foreach ($results as $result) {
			...
			$status_top = $result['status_top'];
            $status_hit_new = '';
            if ($status_hit) {
                $status_hit_new .= $this->language->get('text_status_hit');
            }
            if ($status_new) {
                if (!empty($status_hit_new)) $status_hit_new .= "/";
                $status_hit_new .= $this->language->get('text_status_new');
            }
            if ($status_top) {
                if (!empty($status_hit_new)) $status_hit_new .= "<br />";
                $status_hit_new .= $this->language->get('text_status_top');
            }
		...
		$this->data['column_hit_new'] = $this->language->get('column_hit_new');
        ...
        // filter_category_id start
		if (isset($this->request->get['filter_category_id'])) {
			$url .= '&filter_category_id=' . $this->request->get['filter_category_id'];
		}
        //filter_category_id end
		...
		$this->data['sort_sales_count'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.sales_count' . $url, 'SSL');
  	...
  	private function getForm() {
    	...
    	$this->data['entry_sales_count'] = $this->language->get('entry_sales_count');
    	$this->data['entry_status_hit'] = $this->language->get('entry_status_hit');
    	$this->data['entry_status_new'] = $this->language->get('entry_status_new');
    	$this->data['entry_status_top'] = $this->language->get('entry_status_top');
    	...
    	if (isset($this->request->post['sales_count'])) {
      		$this->data['sales_count'] = $this->request->post['sales_count'];
    	} else if (isset($product_info)) {
			$this->data['sales_count'] = $product_info['sales_count'];
		} else {
      		$this->data['sales_count'] = 0;
    	}

    	if (isset($this->request->post['status_hit'])) {
      		$this->data['status_hit'] = $this->request->post['status_hit'];
    	} else if (isset($product_info)) {
			$this->data['status_hit'] = $product_info['status_hit'];
		} else {
      		$this->data['status_hit'] = 0;
    	}

    	if (isset($this->request->post['status_new'])) {
      		$this->data['status_new'] = $this->request->post['status_new'];
    	} else if (isset($product_info)) {
			$this->data['status_new'] = $product_info['status_new'];
		} else {
      		$this->data['status_new'] = 0;
    	}

    	if (isset($this->request->post['status_top'])) {
      		$this->data['status_top'] = $this->request->post['status_top'];
    	} else if (isset($product_info)) {
			$this->data['status_top'] = $product_info['status_top'];
		} else {
      		$this->data['status_top'] = 0;
    	}
		...
		foreach ($product_videos as $product_video) {
			...
			$this->data['product_videos'][] = array(
				...
				'preview' => $this->model_tool_image->resize($image, 120, 90)
			);
		}
