...
class ControllerDesignLayout extends Controller {
	...
	private function getForm() {
		...
		if (isset($this->request->post['image_order'])) {
			$this->data['image_order'] = $this->request->post['image_order'];
		} elseif (isset($layout_info)) {
			$this->data['image_order'] = $layout_info['image_order'];
		} else {
			$this->data['image_order'] = 0;
		}
		...
		if (isset($this->request->post['layout_image'])) {
			$layout_images = $this->request->post['layout_image'];
		} elseif (isset($layout_info)) {
			$layout_images = $this->model_design_layout->getLayoutImages($this->request->get['layout_id']);
		} else {
			$layout_images = array();
		}

		$this->data['layout_images'] = array();

		foreach ($layout_images as $layout_image) {
			if ($layout_image['image'] && file_exists(DIR_IMAGE . $layout_image['image'])) {
				$image = $layout_image['image'];
			} else {
				$image = 'no_image.jpg';
			}

			$this->data['layout_images'][] = array(
				'layout_image_id'          => $layout_image['layout_image_id'],
				'sort_order'               => $layout_image['sort_order'],
				'image'                    => $image,
				'preview'                  => $this->model_tool_image->resize($image, 190, 68)
			);
		}

		$this->data['no_image'] = $this->model_tool_image->resize('no_image.jpg', 190, 68);
