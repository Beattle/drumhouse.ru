...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
        ...
        if (isset($data['sales_count'])) {
            $sql .= ", sales_count = '" . (int)$data['sales_count'] . "'";
            $sql .= ", status_hit = '" . (int)$data['status_hit'] . "'";
            $sql .= ", status_new = '" . (int)$data['status_new'] . "'";
            $sql .= ", status_top = '" . (int)$data['status_top'] . "'";
        }
		...
		if (isset($data['product_video'])) {
			foreach ($data['product_video'] as $video) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_video SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape($video['image']) . "', video = '" . $this->db->escape($video['video']) . "'");
			}
		}
