...
class ControllerCommonHeader extends Controller {
	protected function index() {
        ...
        $image_order = $this->model_design_layout->getLayoutImagesOrder($layout_id);
        $backgrounds = array();
        $backgrounds = $this->model_design_layout->getLayoutImages($layout_id);
        if ($backgrounds) {
            if ($image_order && count($backgrounds) > 1) {
                shuffle($backgrounds);
            }
            $bg_image = $backgrounds[0]['image'];

		    if (file_exists(DIR_IMAGE . $bg_image)) {
			    $this->data['bg_image'] = $server . $bg_image;
		    } else {
			    $this->data['bg_image'] = '';
		    }
        } else {
        ...
        if ($banners) {
    		$results = $this->model_design_banner->getBanner($banners[0]['banner_id']);
	    	...
	    	foreach ($results as $result) {
		    	if (file_exists(DIR_IMAGE . $result['image'])) {
			    	$this->data['banners'][] = array(
				    	'title' => $result['title'],
					    'link'  => $result['link'],
					    'image' => $this->model_tool_image->resize($result['image'], $banners[0]['width'], $banners[0]['height'])
				    );
			    }
		    }
        }
		...
		$this->data['contacts_icq']         = $this->config->get('contacts_icq');
      	...
      	$main_menu = $this->language->get('main_menu');
        $items = explode('|', $main_menu);

        // abount us
        $this->data['menu'][] = array(
            'title' => isset($items[0])? $items[0] : 'no title',
            'href'  => $this->url->link('information/information', 'information_id=4')
        );
        // catalog
        $this->data['menu'][] = array(
            'title' => isset($items[1])? $items[1] : 'no title',
            'href'  => $this->url->link('product/catalog')
        );
        // news
        $this->data['menu'][] = array(
            'title' => isset($items[2])? $items[2] : 'no title',
            'href'  => $this->url->link('news/category', 'catid=1')
        );
        // shipping
        $this->data['menu'][] = array(
            'title' => isset($items[3])? $items[3] : 'no title',
            'href'  => $this->url->link('information/information', 'information_id=6')
        );
        // payment
        $this->data['menu'][] = array(
            'title' => isset($items[4])? $items[4] : 'no title',
            'href'  => $this->url->link('information/information', 'information_id=7')
        );
        // video
        $this->data['menu'][] = array(
            'title' => isset($items[5])? $items[5] : 'no title',
            'href'  => $this->url->link('video/category')
        );
        // articles
        $this->data['menu'][] = array(
            'title' => isset($items[6])? $items[6] : 'no title',
            'href'  => $this->url->link('news/category', 'catid=2')
        );
        // contacts
        $this->data['menu'][] = array(
            'title' => isset($items[7])? $items[7] : 'no title',
