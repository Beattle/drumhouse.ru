...
class ControllerCommonSeoPro extends Controller {
		...
		if (isset($this->request->get['_route_'])) {
			...
			if ($query->num_rows == sizeof($parts)) {
				...
				foreach ($parts as $part) {
					...
					} elseif ($url[0] == 'video_category_id') {
    					if (!isset($this->request->get['vcatid'])) {
    						$this->request->get['vcatid'] = $url[1];
    					} else {
    						$this->request->get['vcatid'] .= '_' . $url[1];
    					}
    				} else {
			...
			} elseif(isset($this->request->get['vcatid'])) {

                if (isset($this->request->get['video_id'])) {
    			    $this->request->get['route'] = 'video/video';
                } else {
                    $this->request->get['route'] = 'video/category';
                }
			...
			switch ($key) {
				...
				case 'catid':
					...
					foreach ($newscategories as $newscategory) {
					    $queries[] = 'news_category_id=' . $newscategory;
					}
					unset($data[$key]);
					break;

				case 'vcatid':
					$videocategories = explode('_', $value);
					foreach ($videocategories as $videocategory) {
					    $queries[] = 'video_category_id=' . $videocategory;
					}
					unset($data[$key]);
					break;
	...
	private function validate($link) {
		$get = array('path', 'catid', 'product_id', 'manufacturer_id', 'category_id', 'information_id', 'news_id', 'vcatid', 'video_id');
