...
class ControllerExportYml2 extends Controller {
	...
	/**
	 * Положить сформированный yml в файл yandex_yml.xml
	 *
	 */
...
*/
	...
	/**
	 * Формирование YML файла и запись на лету в файл
	 *
	 * @return string
	 */
	...
	private function writeYml($filename) {
		$fp = fopen($filename, 'w');
	...
	/**
	 * Редирект на сформированный файл выгрузки
	 *
	 */
	public function index() {
	    $this->redirect(HTTP_SERVER.'download/yandex_yml.xml');
	}
...
//		}
    ...
    private  function getPathByProduct($product_id) {
        ...
        if (!isset($path[$product_id])) {
            $query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $product_id . "' ORDER BY main_category ASC");
