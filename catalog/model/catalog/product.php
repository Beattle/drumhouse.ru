...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			return array(
				...
				//'name'             => $query->row['name'],
				'name'             => mb_strtoupper($query->row['name'], 'UTF-8'),
				...
				'status_hit'       => $query->row['status_hit'],
				'status_new'       => $query->row['status_new'],
				'status_top'       => $query->row['status_top'],
    ...
    public function getProduct_by_sku($sku) {
		...
		if (!$product_data) {
			...
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
					$sql .= " ORDER BY p.status_top DESC, LCASE(" . $data['sort'] . ")";
				} else {
					$sql .= " ORDER BY p.status_top DESC, " . $data['sort'];
				}
			} else {
				$sql .= " ORDER BY p.status_top DESC, p.sort_order";
			}
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) {
            ...
            $sql .= "AND p.status_new = '1' ";
            $sql .= "AND p.date_available <= NOW() ";
            $sql .= "AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ";
            $sql .= "ORDER BY p.status_top DESC, p.date_added DESC LIMIT " . (int)$limit;
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) {
            ...
            $sql .= "LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) ";
            $sql .= "WHERE p.status = '1' ";
            $sql .= "AND (p.status_hit = '1' OR p.sales_count >= '" . (int)$this->config->get('config_hit_status_limit') ."') ";
            $sql .= "AND p.date_available <= NOW() ";
            $sql .= "AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ";
            $sql .= "ORDER BY p.status_top DESC, p.status_hit DESC, p.sales_count DESC LIMIT " . (int)$limit;
