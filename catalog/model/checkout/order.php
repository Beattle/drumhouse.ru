	...
	public function confirm($order_id, $order_status_id, $comment = '') {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			foreach ($order_product_query->rows as $order_product) {
                ...
                $sql .= "quantity = (quantity - " . (int)$order_product['quantity'] . "), ";
                $sql .= "sales_count = (sales_count + " . (int)$order_product['quantity'] . ") ";
                $sql .= "WHERE product_id = '" . (int)$order_product['product_id'] . "' ";
                $sql .= "AND subtract = '1'";
