...
class ModelSaleOrder extends Model {
    ...
    public function updateOrder($order_id, $data) {
        ...
        $product_query = $this->db->query("SELECT product_id, quantity FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = 'sub_total', title = 'Сумма', value = '" . floatval($total)."', text = '" . floatval($total) . " р.', sort_order = 1");
        $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = 'shipping', title = '".$this->db->escape($data['shipping_method'])."', value = '" . floatval($shipping_price)."', text = '" . floatval($shipping_price) . " р.', sort_order = 2");
        $total+= floatval($shipping_price);
        $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = 'total', title = 'Итого', value = '" . floatval($total)."', text = '" . floatval($total) . " р.', sort_order = 3");
        ...
        */
        ...
        if ($data['fax'] != '') {
            ...
            if ($customer_id > 0) {
                $this->db->query("UPDATE " . DB_PREFIX . "order SET  fax = '" . $this->db->escape($data['fax']) . "' WHERE customer_id='" . (int)$customer_id . "' AND fax=''");
                $this->db->query("UPDATE " . DB_PREFIX . "customer SET  fax = '" . $this->db->escape($data['fax']) . "' WHERE customer_id='" . (int)$customer_id . "' AND fax=''");
            }
        }        
        // -- Убогий костыль по изменению доп.телефона в заказах от этого же юзера
    ...
    public function notifyUpdateOrder($order_id, $order_info) {
		...
		//$template->data['invoice_no'] = $order_info['invoice_no'];
		...
		foreach ($order_product_query->rows as $product) {
			...
			$template->data['products'][] = array(
				...
				'href'     => 'http://'.$_SERVER['HTTP_HOST'].'/index.php?route=product/product&product_id='.$product['product_id']
			);
		}
		...
		foreach ($template->data['totals'] as $key=>$total) {
		    $template->data['totals'][$key]['text'] = $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']);
		}
    ...
    public function getOrder($order_id) {
        ...
        if ($shipping_query->num_rows) {
            $shipping_price = $shipping_query->row['value'];
        }
        else {
            $shipping_price = 0;
        }
    ...
    private function notifyCustomer($order_id, $order_info, $data) {
